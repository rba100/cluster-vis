...
with col1:
    ...
    with st.expander("Automatic cluster identification", expanded=False):
        ...
        st.session_state.removeConceptText = st.text_input("Remove concept from cluster names")
    ...
    with st.expander("Help", expanded=False,):
        ...
        st.caption("'Remove concept from cluster names'. If you have a concept that is common to all the items you can enter it here and then clustering will try to look at the sentiment in the text items that does not relate to that concept. For example, if you have a list of comments about 'car problems' you don't want the clustering to be dominated by the word 'car'. This is a feature that can really mess up the clustering if you enter text which isn't common to all text items because they will be modified as if they were which could take them literally anywhere in multidimentional vector space.")
